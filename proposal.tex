...
Others infer types just by looking at the expressions, and others simply don't know what the type of the expression is until they encounter them in runtime.
...
If you have a static type system, you can guarantee that there will not be runtime type errors, but from the perspective of a programmer, a simple solution can become tedious because of the lack of flexibility.
...
What Siek and Taha \cite{Siek2006GradualTF} intuited was that this program should be accepted by a gradual type system, because declaring the argument $x$ as $?$ means that $x$ can be any type, thus it can be an \texit{int}.
...
Type consistency allows a gradual type checker to know when to accept a program; if every function argument is consistent with its expected type, then there should be no problem.
...
For the statics semantics, types, predicates and functions are lifted into their gradual counterpart in such a way so that they; 1) work just the same as the static version when when all types are specified, and 2) they accept $?$ as a type. This new gradual version of the lenguage complies the properties discussed in Siek et al. \cite{siek_et_a:LIPIcs.SNAPL.2015.274} and type safety.

While in the statics part we care about if our typing holds, during runtime we care about why it still holds.
That's why a new system is built that introduces \textit{evidence}, that in basic terms is information about an unknown type that is updated using other types in the program.

To elaborate on the intuition, let's analyze how the following program should run:
...
The function \textit{add1} is declared $?$ (we are not specifing its type), so the expression ***add1 true*** should be accepted.
The dynamic part starts 

%No se con cuanto detalle explicar como funciona AGT. Me imagino que los dynamics los tengo que explicar más a fondo, porque con eso voy a trabajar.
