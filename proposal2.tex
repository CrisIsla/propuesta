...
An alternative to this approach is Abstracting Gradual Typing (AGT).
...
which in simple words means that $\tilde{R}$ of two gradual types $G_1$ and $G_2$ holds if there exist two static types $T_1$ and $T_2$ in the sets that each gradual type represents where $R(T_1, T_2)$ holds.
...
Using the definition of gradual types that represent a set of static types, this judgement holds because $\gamma(?)$ contains various numbers that are greater or equal than 1 (in other words, there exist at least one type that suffies the lifting of the $\tilde\leq$ relation)
...
\begin{enumerate}
  \item First we calculate the two set of static type pairs $T_1$ and $T_2$ where the relation $R$ holds.
...
For this, we need to check inside the evidences $\epsilon_1$ and $epsilon_2$, and see if there exists a subset of TYPEs in the intersection between the concretization of each part of the innermost part of this evidences, where the relation $R$ holds.
...
Finally, because we could calculate the consistent transitivity, the judgement $10 \tilde\leq 20$ holds:
