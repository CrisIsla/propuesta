...
Gradual typing has emerged as one of the main options to reconcile this opposing paradigms, allowing a single language to support both disciplines seamlessly by introduccing the unknown type (denoted by $\?$), a type that represents every type.
...
\noindent Now, this program should type check because the functions argument can be any type, but if no runtime checks are applied, this program would most probably end in segmentation fault.
...
Grift's monotonic configuration (GriftMS) makes it so that every time a reference is casted to a more precise type, the original heap cell is updated to display this change.
...
GrEv implements the GTLC+ language, which syntax is presented in Figure~\ref{fig:lang-syntax}.
...
This program type checks because $bool \sim \?$, but in runtime, as the function tries to add the argument with itself, we have a chain:
...
In general terms, evidence has its own types $ETYPE$, but in GrEv's case, evidence types are the same as gradual types.
...
In order to understand how evidence should work and evolve, we can list the operations needed and use them to briefly overview the runtime evolution of the example program of the previous section.
...
\begin{enumerate}
  ...
  \item \textbf{\textit{Dom and Cod:}} For ``$\rightarrow$'' types, consistent transitivity must be checked between its arguments and the functions $dom$. If this judgement holds, then the resulting value should have evidence with the type of the functions $cod$.
\end{enumerate}
...
GrEv differentiate them by whether they are immediate or stored in the heap (boxed).
...
\begin{enumerate}
  ...
  \item \textbf{\textit{Consistent transitivity:}} Whenever a check between two large structural types is made, consistency between their tree-like structures must be computed.
    In particular, to check consistency between two ``$\rightarrow$'' types $G_1$ and $G_2$, $dom(G_1) ~ dom(G_2)$ and $cod(G_1) ~ cod(G_2)$ must be checked recursively.
\end{enumerate}

\section{Proposed solutions}

The proposed solutions aim to optimize GrEv's evidence memory representation in order to explore the impact the presented problems have over the compilers performance, and to improve it in this specific cases.
In order to accomplish this, new memory representations for evidence must be implemented and tested.
As an initial research, the following optimization have been considered:
...
\begin{itemize}
  \item \textbf{\textit{Hash consing:}} Hash consing is an optimization that aims to improve the memory allocation problem.
    ...
    \noindent Grift implements this optimization for structural types on its Monotonic mode (Grift\/MS).

  \item \textbf{\textit{Consice Binary Object Representation and Compact Data Structures:}} The problem with hash consing is that it does not provide an intuitive way of calculating two of the main operations needed for structural types: $dom$ and $cod$.
    ...
    Consice Binary Object Representation (CBOR) and Compact Data Structures \cite{Navarro_2016} are good candidates.
    Creating a new memory representation based on these candidates may offer more flexibility over hash consing at the expenses of using more memory.
    Either way, more reaserch on these memory representations is needed in order to fully see its potentials and weaknesess.
\end{itemize}
...
The main objective of this proposal is to optimize GrEv's evidence current memory representation using a more suitable data structure, that is more efficient, space and timewise, on evidence specific operations: $cod$, $dom$, \textit{elaboration} and \textit{consistent transitivity}.
...
\begin{enumerate}
  ...
  \item \textbf{Performance evaluation via GTP:} The standarization of GrEv's benchmarks suite will give better insight on the performance of the compiler on its current state, and also on any changes, extensions or experiments made in the future.
\end{enumerate}
