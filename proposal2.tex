...
In the world of software engineering we often find ourselves in the dilemma of choosing between two distinct tools: a flexible dynamically typed language, or a reliable statically typed one.
...
Gradual typing has emerged as one of the main options to reconcile these opposing paradigms, allowing a single language to support both disciplines seamlessly by introducing the unknown type (denoted by $\?$), a type that represents every type.
...
To illustrate how gradual typing works, let us gradualize the following fully static program:
...
\noindent The program presents a function with type $int \rightarrow int$ being applied to a value with type $bool$.
This program should not type check because $add1$ expects an $int$ as an argument, but a $bool$ is being given.
We can make this function type check by changing the argument type of the function to $\?$

\begin{lstlisting}
let add1: ? -> int = fun x -> x + 1 in 
  add1 true;
\end{lstlisting}

\noindent Now, this program should type check because the function's argument can be any type, but if no runtime checks are applied, this program would most probably end in segmentation fault.
Gradual typing statically checks for type errors, and when types are not precise enough to ensure safety, runtime checks are inserted.

Despite the elegant idea, the practical realization of gradual typing comes with significant engineering challenges.
...
To date, there exist two main approaches on how to implement gradual compilers.
Coercion-based gradual typing uses coercions (specialized data structures representing the path between source and target types) to check type consistency.
The Grift compiler \cite{grift} stands as the reference implementation of this approach.
...
Following the previous example, to check if the argument given to the function call is correct, its evidence would be compared using an specific operation with the evidence of the function in order to see if it can be applied.
If this operation succedes, then each evidence (the argument and the funcion one) would be updated to a more precise version.
Being more specific, the function's evidence would be updated so that it tell us that a boolean (the type of the argument) is accepted by the function.

The GrEv compiler \cite{grev} represents the first attempt to build a high-performance compiler based on evidence.
...
Despite its great results, there still is a lot of room for improvement, and the question about whether these results are because of the simplistic nature of the scenario or for other reasons arises.
...
In this model, the static semantics of the gradual language are elaborated into an intermediate cast calculus and these casts are then compiled into coercions, which in simple terms are a runtime representation of casts.
One of the main challenges in these systems is that coercions can be accumulated.
For example, consider a function that is passed from a typed module to an untyped module and back again multiple times.
A straightforward implementation incurs a new coercion at every boundary crossing, resulting in inefficient memory utilization.
Grift mitigates this problem via coercion composition.
Whenever a new coercion is applied to an already coerced value, the runtime system attempts to normalize the composition.
If the coercions are inverses (casting $\mathsf{int}$ to $\?$ and then back to $\mathsf{int}$, for example), they cancel each other out, and the coercion is removed.
...
The monotonic configuration of Grift (GriftMS) makes the heap cell itself track the most precise type it has been cast to.
...
GrEv implements the GTLC+ language, whose syntax is presented in Figure~\ref{fig:lang-syntax}.
This compiler represents a paradigm shift, implementing the runtime semantics of AGT directly.
...
The key idea in the AGT framework is to view gradual types as abstractions of sets of static types, formally defined by a Galois connection.
These connections define two functions, the concretization function $\gamma$ and the abstraction function $\alpha$, both functioning as a connection between sets which, in this case, are GTYPE (the set of all gradual types), and $P(TYPE)$ (the set of all sets that can be created from static types).
...
With this definition we can think about \textit{precision}.
...
In a static system there could be different type relations, like equality, subtyping, containment, etc.
What AGT proposes is that, in order to get the static semantics of a gradual system, these relations must be lifted into their gradual counterparts.
...
We say that two gradual types $G_1$ and $G_2$ are consistent if there exists a type $T$ that belongs to the concretization of both $G_1$ and $G_2$ (or in other words, if there exist two static types in each concretization that fulfill the equality relation).
...
\noindent but $int ~ bool$ does not hold.

\subsection{Evidence and Consistent Transitivity}
\label{sec:evconsistent}

AGT provides a direct dynamic semantics of gradual programs.
...
Let us introduce the following example program:

\begin{lstlisting}
let f: ? -> int = fun x -> x + x in 
  f true
\end{lstlisting}

\noindent In this example, the function $\mathsf{f}$ expects an $\mathsf{\?}$ and a value of type $bool$ is given.
This program type checks because $bool \sim \?$, but at runtime, as the function tries to add the argument with itself, we have a chain:

$$bool \sim \? \land \? \sim int$$

\noindent The way to try to justify this judgment is through the use of evidence.

Evidence is a runtime object that carries information about the type of some value. It evolves during runtime in order to give the current most precise information about it.
In the example, statically we cannot know the exact type of the argument of the function, so we have to check during runtime if the judgement holds with the value given to it.
...
In GrEv, every value is an \textit{evidence value}. This means that every value carries an evidence in order to justify judgements.
Evidence starts from an initial state derived from the type given statically to the value associated to it, and evolves whenever more precise information about this type is available.
When a judgement between two values is checked, their evidences are ``combined'' and, depending if the combination succeedes or not, the program continues or halts with a runtime error.

In order to understand how evidence is introduced and evolves, we list the operations needed and use them to briefly overview the runtime evolution of the example program of the previous section.

\begin{enumerate}
  \item \textbf{\textit{Elaboration:}} First, the initial evidence of each value in the program is calculated.
    Operations are also elaborated, insering ascriptions and evidences to make sure that the values being operated are consistent with what the operation expects.
  \item \textbf{\textit{Consistent Transitivity:}} Whenever a judgement between two values needs to be justified, their evidences are combined using \textit{consistency transitivity} (denoted by $\circ$).
    In order to justify a judgement, we justify that transitivity holds for an specific case of consistency (for example, between $int\sim \?$ and $\? \sim bool$).
    If the judgements holds, the resulting evidence is the result of this operation between the evidences. If the judgement is not justified, the program halts with a runtime error.
  \item \textbf{\textit{Inversion functions:}} For function types, consistent transitivity must be checked between its arguments and the function's domain. If this judgement holds, then the resulting value should have evidence with the type of the function's codomain.
\end{enumerate}

\noindent Using these operations, the example program should evolve like this:

\begin{itemize}
  \item The elaboration of the program should initialize the evidence of the function \textit{f} and its argument as $\langle \? \rightarrow int\rangle$ and $\langle bool\rangle$, respectively.
    Also, the body of the function has the operation $+$, so each operand is ascribed to $int$.
    The function application would be as follows:
  \begin{lstlisting}
  $\langle \? -> int\rangle$(fun $x$ -> $\langle int\rangle x :: int + \langle int \rangle x :: int$) $\langle bool \rangle$ true
  \end{lstlisting}
  \item Then, the evidence of the argument should be combined using \textit{consistent transitivity} with the \textit{dom} of the evidence of the function \textit{f} (in this case, $\langle ? \rangle$), and the result should be ascribed to the functions \textit{cod}.
  \item Because $\langle bool \rangle$ is not consistent with $\langle int\rangle$ (this last evidence came from the ascription of the $+$ operation), the judgement does not hold and the program halts with a runtime error.
\end{itemize}

If the program had the function applied to an $int$, then this last judgement would have been justified, and the resulting value of the application would have had evidence $\langle int \rangle \circ \langle cod(f)\rangle = \langle int \rangle$
...
As mentioned before, values are evidence values.
GrEv differentiates them by whether they are immediate or stored in the heap (boxed).
Immediate values (integers and booleans) are tagged, and because evidence is represented as gradual types, tags also function as evidence for this kind of values.
Boxed values (as shown in Table~\ref{tab:values}) have a specific field in memory that saves its evidence alongside their respective information.
This evidence is stored as an structure that contains an id (there is one for every type in the language) and some other information regarding the type asociated.
For example, the evidence of a function is stored in a structure that contains its id, the arity of the function, and an array with the evidences of each argument and return type.
When the evidence is updated into a more precise type, this can result in a tree-like structure, for example, when the new type is another arrow type.

Depending on the configuration (as shown in Table \ref{tab:values}), values will be stored next to their evidence (monotonic configurations), or in a proxy that contains evidence and a pointer to the values (guarded configurations).
...
To illustrate the semantic each of the configurations supports, let's analyse the program in Figure~\ref{fig:mono-vs-guarded} that uses references (closures and tuples follow the same logic).
...
As shown in the example of section \ref{sec:evconsistent}, in order to reduce a program containing structural types ($\rightarrow$), $dom$, $cod$ and $\circ$ operations must be applied (the elaboration process is static).
As these types become more complex, their tree-like representation becomes deeper, and two problems emerge:
\begin{enumerate}
  \item \textbf{\textit{Memory allocation:}} Memory allocation is a slow operation.
    In order to store the evidence of a complex structural type, many memory allocations must be made (one for each node in the tree).
  \item \textbf{\textit{Consistent transitivity:}} Whenever a check between two structural types is made, consistency between their tree-like structures must be computed.
    This becomes increasingly expensive as the size of the structures increase.
\end{enumerate}

\section{Proposed solutions}

The proposed solutions aim to optimize GrEv's evidence memory representation to improve the compiler's performance in these specific cases.
In order to accomplish this, new memory representations for evidence must be implemented and tested.
As an initial research, the following optimizations have been considered:

\begin{itemize}
  \item \textbf{\textit{Memoization:}} Memoization is an optimization that consists of caching the result of expensive function calls and returning the cached result whenever the same call is made in the future.
    This optimization could help when evaluating consistent transitivity between large structural types.
  \item \textbf{\textit{Hash consing:}} Hash consing is an optimization that aims to improve the memory allocation problem.
    In simple terms, it consists of hashing structural types for better memory management and equality (or in this case consistency) checking.
    Every time a structural type is needed, a hash of its contents is calculated.
    When the same type appears in another region, instead of allocating more memory, the hash of the type is looked on the table.
    If it exists, then there is no need to allocate more memory, and the pointer to the type is shared.
    This makes it so that the comparison between two types is the same as the comparison of its pointers.

    \noindent Grift implements this optimization for structural types on its Monotonic mode (Grift\/MS) \cite{almahallawi:phd}.

  \item \textbf{\textit{Different memory representations for boxed values:}} The problem with hash consing is that it does not provide an intuitive way of calculating inversion operations needed for structural types.
    Other memory representations may not be as space-efficient as hash-consing, but can provide a middle term between space and time efficiency.
    Concise Binary Object Representation (CBOR) and Compact Data Structures \cite{Navarro_2016} are good candidates, though recent experiments carried out by Jose Luis Romero in his MSc thesis show that some tuning might be necessary to make them effective in this setting.
    Creating a new memory representation based on these candidates may offer more flexibility over hash consing at the expense of using more memory.
    Either way, more research on these memory representations is needed in order to fully see its potentials and weaknesses.
\end{itemize}
...
\begin{itemize}
  \item To what extent can optimized memory representations for evidence, specifically through optimizations like memoization and hash consing, and different data structures that suit better on evidence specific needs, improve the performance of GrEv?
\end{itemize}
% The research questions regarding this thesis are the following:
%
% \begin{itemize}
%   \item Do the proposed optimizations on evidence memory representation help to reduce the overhead of the current implementation of GrEv?
...
The current implementation of GrEv's evidence significantly affects on its performance, and it is possible to improve it by using an implementation that is optimized for the operations needed. 

\section{Main Objective}

The main objective of this proposal is to optimize GrEv's evidence current memory representation using a data structure that suits better on evidence specific operations.

\section{Methodology}

To accomplish such goals, the following steps will be followed:
...
\begin{itemize}
  \item \textbf{Benchmark Analysis.} The GTP benchmarks suite \cite{GTP} consists of 21 benchmarks, each containing and measuring different characteristics of the language, some of which are not currently implemented on GrEv.
    There will be a selection of the most important benchmarks from this suite for the current version of GrEv, and also an analysis of which features will be implemented depending on the relevance of the benchmarks that GrEv currently can not support.
  \item \textbf{Language Modularization.} The current state of GrEv's code can be improved to better support modular evidence memory representations.
    There will be an analysis on what the best way to modularize GrEv's evidence implementations, so that it's easier to add and change between different versions.
  \item \textbf{Language Extension.} Depending on the selected benchmarks, the necessary features will be implemented on GrEv.
...
\begin{itemize}
  \item \textbf{Optimization Evaluations.} After implementing each optimization, there will be experimental evaluation to check how this affects GrEv's performance. The optimized versions will be compared between themselves, with Grift (the state-of-the-art gradual typing compiler) and GrEv's different configurations, depending on the context. These comparisons will consider time and space performances.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Performance evaluation via GTP:} The standardization of GrEv's benchmarks suite will give better insight on the performance of the compiler on its current state, and also on any changes, extensions or experiments made in the future.
\end{enumerate}
